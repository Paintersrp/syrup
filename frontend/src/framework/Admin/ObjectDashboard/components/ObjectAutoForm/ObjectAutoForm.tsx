...
const ObjectAutoForm: React.FC<ObjectAutoFormProps> = ({
  ...
  useEffect(() => {
    setReady(false);
    if (!location.state) {
      ApiAxiosInstance.get(`/get_models${endpointUrl}`)
        .then((response) => {
  ...
  useEffect(() => {
    ApiAxiosInstance.get(`/get_metadata${endpointUrl}`).then((response) => {
  ...
  const handleUpdatePreview = (model_name, query_params) => {
    ...
    if (model_name !== "None") {
      ApiAxiosInstance.get("preview-data/", {
        params: {
  ...
  return (
    ...
    >
      {modelMetadata.info_dump && (
        <InfoMenu textItem={modelMetadata.info_dump} />
      )}
