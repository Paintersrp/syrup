...
export const TransferList: React.FC<TransferListProps> = ({ leftItems, rightItems, ...rest }) => {
  ...
  const handleMoveRight = () => {
    const updatedLeftItems = leftItems.filter((item) => !selectedLeftItems.includes(item.id));
  ...
  const handleMoveLeft = () => {
    ...
    const updatedRightItems = rightItems.filter((item) => !selectedRightItems.includes(item.id));
  ...
  return (
    <Base className="transfer-list-container" {...rest}>
